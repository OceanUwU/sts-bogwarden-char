    ...
    public static class SpiritualJourneyPower extends AbstractBogPower {
        ...
        public void onPlayCard(AbstractCard card, AbstractMonster m) {
            if (card.type == AbstractCard.CardType.ATTACK) {
                flash();
                final int decrease = amount;
                att(new AbstractGameAction() {
                    public void update() {
                        isDone = true;
                        owner.decreaseMaxHealth(decrease);
                        AbstractDungeon.effectsQueue.add(new DamageNumberEffect(owner, owner.hb.cX, owner.hb.cY, decrease));
                        for(int i = 0; i < 50; i++)
